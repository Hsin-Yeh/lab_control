"""Continuous GSM20H10 output + monitoring experiment."""
...
def run_gsm_monitor(
    ...
    """Run continuous output and monitoring with GSM20H10.
    ...
    Parameters:
        ...
        duration_s: Total monitoring duration (units: s).
    ...
    logger = setup_logger("gsm_monitor", "logs/gsm_monitor.log")
    ...
    with smu:
        try:
            ...
            smu.output_on()
            logger.info(
                "GSM monitor started: V=%.6f V, I_limit=%.6f A, interval=%.3f s, duration=%.3f s",
                source_voltage_v,
                current_limit_a,
                interval_s,
                duration_s,
            ...
            while True:
                if abort_event is not None and abort_event.is_set():
                    logger.warning("GSM monitor aborted by user")
                    break
                ...
                if abort_event is not None and abort_event.wait(interval_s):
                    logger.warning("GSM monitor aborted during wait")
                    break
    ...
    if results:
        save_csv(results, str(out_subdir / "gsm_monitor.csv"))
        logger.info("Saved GSM monitor data: %d rows", len(results))
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    Parameters:
        None (units: none).
    """
    parser = argparse.ArgumentParser(description="Run GSM continuous monitor")
