"""Long-term resistance logging experiment tab UI."""
...
from gui.workers import ResistanceLogWorker


class ResistanceLogTab(QWidget):
    """GUI tab for long-term resistance logging.
    ...
    def __init__(self, config_path: str, parent: QWidget | None = None) -> None:
        ...
        self._worker: ResistanceLogWorker | None = None
        ...
        self._duration_s.setValue(120.0)

        self._compliance_stop = QCheckBox("Stop on compliance", self)
        self._compliance_stop.setChecked(True)
        ...
        form.addRow("", self._compliance_stop)
        ...
        self._plot.setLabel("left", "Resistance", units="Ohm")
        self._plot.setLabel("bottom", "Elapsed Time", units="s")
        self._curve = self._plot.plot([], [], pen=pg.mkPen("g", width=2), symbol="o")
    ...
    def _on_run(self) -> None:
        """Start resistance logging worker.

        Parameters:
            None (units: none).
        """
        self._worker = ResistanceLogWorker(
            ...
            compliance_stop=bool(self._compliance_stop.isChecked()),
        )
    ...
    def _on_finished(self, results: list) -> None:
        ...
        resistance = [float(row.get("resistance_Ohm", 0.0)) for row in results]
        self._curve.setData(elapsed, resistance)
