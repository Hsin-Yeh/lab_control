"""Tests for GSM monitor experiment."""
...
def test_run_gsm_monitor_creates_output(tmp_path):
    """GSM monitor run returns records and writes CSV.
    ...
    csv_files = list(output_dir.glob("*/gsm_monitor.csv"))
    assert csv_files


def test_run_gsm_monitor_respects_abort(tmp_path):
    """GSM monitor honors pre-set abort event.
...
def test_run_gsm_monitor_emits_progress(tmp_path):
    """GSM monitor emits progress callback updates.
